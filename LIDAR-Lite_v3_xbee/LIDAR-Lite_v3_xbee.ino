//NOTE: This code is meant for LIDAR-Lite v3
...
void sendJamPacket()
{
  ...
  int targetID = 13;   // 0 is all sensor ids targeted ////////////////////////////////////////////////// UPDATE
...
void loop() {
  ...
  distance = getLidarLiteDist();
  
  if(distance && (millis() - lastSeen) > 100) {
    
    float frequency = (pulseCount * 1000.0) / (millis() - lastSeen);  // Pulses per second

    if(frequency > 90 && frequency < 110) { // jams sensor when diode reads 90-110 Hz (Can be adjusted)
...
/*
Future work: 
  -Read Freq at a faster/accurate rate, match speed of sensor (receives data every 100ms)
